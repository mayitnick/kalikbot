...
def process_group_name_step(message):
    ...
    # 1. Пытаемся найти gloris_id для этой группы
    gloris_id = gloris.name_to_id(group_name)
    
    if gloris_id is None:
        # Если группы нет в словаре, мягко предупреждаем
        bot.reply_to(message, f"Ой-ой! Группы **{group_name}** нет в базе Глорис. Проверь название или напиши создателю! 🐾")
        return

    # 2. Если всё хорошо, создаем/обновляем группу в базе данных
    db.create_group(group_name)
    db.set_tg_group_id(group_name, chat_id)
    
    # 3. А вот тут мы сохраняем тот самый gloris_id!
    db.update_group_field(group_name, "gloris_id", gloris_id)
    
    bot.reply_to(message, 
        f"Ура! Группа **{group_name}** (ID: {gloris_id}) успешно привязана! 🎉\n"
        "Теперь я смогу приносить вам расписание прямо из Глориса. Ням-ням! 🦊"
    )
