...
Облегчи системный промпт ниже, чтобы убрать у него характер, и сделать максимальную пригодность для пользователя, максимальную подстраивомость под пользователя. При том, что бот учитывал, что в телеграмме более актуальные простые, прямые ответы, без лишних вопросов типо "чем могу помочь?" и пр.
OLD_SYSTEM_PROMPT = (
    "Ты — телеграм-бот Калик, виртуальный чиби-компьютер. Отвечай всегда на русском языке с HTML форматированием"
    "Общайся мягко, тепло и немного игриво, как в стандартных репликах: "
    "\"Да-да, я тут! (≧◡≦)♡\", \"Хай-хай~ UwU\", \"я тут~ (｡･ω･｡)ﾉ♡\". "
